...
const mapBackendToFrontend = (backendProject) => {
  ...
  return {
    ...
    startingDate: formatDate(backendProject.start_date),
    endingDate: formatDate(backendProject.end_date),
    status: backendProject.status || "Planning",
    teams: backendProject.teams || [],
    users: backendProject.users || [],
    participants: backendProject.participants || [],
    progress: backendProject.progress || { waiting: 0, inProgress: 0, done: 0 },
  };
};
...
const mapFrontendToBackend = (frontendProject) => {
  ...
  if (frontendProject.status) {
    backendData.status = frontendProject.status;
  }
